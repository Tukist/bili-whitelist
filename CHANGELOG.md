...
### 信箱卡片不再"绕回来"
「不停上滑会看到之前处理掉的卡片又出现」这个循环修掉了。根因是上滑（取回）只是把卡片搬回队首、**不消费任何东西**，而「暂缓」那张在飞出去的大约 0.35 秒里既没登记也没被摘除，落地时又被重复登记一次——于是同一张卡可以被无限次取回。

- 现在**一次「暂缓」只欠一次取回**：卡片本体只存一份，另用一张"票据"记着"这张还欠一次取回"；取回时票据被消费掉。交替滑任意多轮，票据最终必然排空。
- 被左滑跳过 / 右滑加入的卡彻底出局，不会再被上滑或后台回读复活。
- **空手取回不再静默**：没有可取回的卡片时会明确提示一句，而不是"上滑了但什么都没发生"（仍遵守设置里「界面提示」的总开关）。
- 顺带修：新抓到的视频以前会被追加到**队尾**、与发布时间顺序脱钩，现在按发布时间插入（但**绝不插到当前这张之前**，不会把你正在看的那张换掉）。

### 设置里三处小字压到两行
「启动行为」90 → 31 字、「界面提示」99 → 32 字、「写操作」**243 → 40 字**。删掉的是举例与操作细节，安全信息留着：仍在读剪贴板、仍只存本机；关掉提示条后错误与「撤销」提示照旧显示；写操作仍标**默认关闭**、仍警告**投币不可撤回**，并多留了一句「（会立刻公开）」。

### 合集封面可以直接从相册选图了
- **封面改成本机图片**：编辑对话框里多一个「从相册选图」，选完**就地预览**；「移除本机封面」回落到原来的 URL。
- **图片只存在本机**：拷进 App 私有目录，映射存在本机偏好里，**一个字节都不会进 Gist**（白名单每次写操作都要 PATCH 整份 JSON，塞图片会把它撑爆）；其它设备看不到这张图，属于预期。Gist 里的 `cover` 字段语义不变、仍是可同步的 URL，渲染时**本机图片优先于 URL**。
- 改名 / 移动 / 删除合集时，本机封面会**跟着搬迁或一并清理**，不会留下错位的封面或孤儿文件。
- 顺带修两个坑：封面 URL 写成 `//host/xx.jpg` 时以前会直接加载失败，现在自动补成 `https:`；编辑对话框以前**没有预览**（v2.38.0 的已知限制），现在有了。
- 相册选择走系统文件选择器，**不需要任何新权限、也没有加新依赖**。

### 修：第一级合集改不了封面和简介
这不是校验问题，是**入口缺失**——首页一级合集卡左滑只有「移动 / 重命名 / 删除」三块（而合集内页的子合集卡是有「封面」的），合集内页的菜单也只有「新建子合集」，所以一级合集在合集页里**无处可改**，只能绕到个人页的管理面板。

- 首页一级合集卡左滑补上第四块「封面」（块宽 60dp，与合集页子合集卡一致）。
- 合集内页的菜单加「编辑封面与简介」，改的就是**当前这个合集**本身——这样任意层级都有就地可改的地方。

### 其它
- 扫正了注释里写错的版本号：4 组共 70 处（`v2.46.0` 那一批实际以 v2.49.0 发布；另有 3 组把开发期补丁号写成了实际发布的 minor 号）。带 `-rN` 的热修重发标记无法证伪是否真发过，保守未动。
- 测试总数 2550 → 2597。

### 已知限制
- 相册选择器与 HEIC 解码只能在真机验证（系统文件选择器 + 各家 ROM 差异）。
- 本机封面不跨设备、不参与 Gist 同步（刻意如此，避免把 JSON 撑爆）。
- 首页一级合集卡现在是四块 60dp，滑开后卡片可见宽度比原来少一点。
- 本机封面不压缩、不去重：同一张图选两次会产生两个文件（上限 10MB）。

---

## v2.49.1 (2026-09-22)

### 登录这次是真的修好了（并补了一条不依赖 WebView 的兜底）
上一版只是让 App 肯承认会话已死，登录本身仍然进不去。真机取证把两个真正的堵点抓出来了：

- **「登录成功」是假的**：服务端早就把那份会话作废了（`-101`），可登录页一打开就读到 WebView 里残留的旧 cookie，立刻判定"登录成功，已解锁 1080P"，返回后收藏夹仍报「登录已失效」——于是「去登录 → 秒弹回 → 再点」无限打转（截图里「登录已失效」与「登录成功」同屏，就是它）。现在**登录一律以服务端点头为准**：拿到 cookie 后先向 B 站确认（`nav` 返回 code=0 且 isLogin=true）才算成功，否则明确告诉你"这份登录状态服务端不认（-101）"并留在登录页；校验因网络问题失败时**默认也不保存**——正是这种"看着对就存下来"的习惯造出了之前那个坏状态。
- **清死会话时没清网页 cookie**：现在两处一起清（本机凭据 + WebView 的 cookie jar），登录页不会再看到残留的那份。
- **新增「粘贴 Cookie 登录」**（个人页 → B 站账号）：完全不依赖 WebView 的一条路。在电脑浏览器里登录 B 站后把整段 cookie 复制过来粘进去，App 会先向 B 站校验再保存，无效的一律不存。**登录页打不开时，这条路照样能登进去。**
- **新增「退出登录」**：带确认框，一次清掉本机凭据与网页登录状态，并如实告知清干净了没有（网页数据没清干净会明确说出来，不会含糊地报"已退出"）。

### 登录页打不开时，现在会告诉你为什么
- 加载看门狗从 30 秒缩到 **10 秒**，不再对着白屏干等半分钟。
- 错误页给两个出口：**「重试」**与**「清除网页数据后重试」**（清 cookie / Web Storage / 缓存并重建 WebView）——后者是治"profile 损坏导致渲染进程崩溃"的。
- 错误里会带上**错误码与主机名**（已脱敏），证书类错误（`-202`）单独成档并直接提示「请关掉代理 / VPN」。**检测到无效证书只提示、绝不放行**——那一页要输手机号和验证码。

### 排查中发现的一个外部因素（可能对你有用）
真机上登录页每次打开约 1.5 秒后渲染进程必崩；换到**电脑上的安卓模拟器**复现同一现象时，紧邻日志是 `tile_manager: tile memory limits exceeded`——说明是登录页那个重型 H5 把 WebView 渲染进程的图块内存撑爆了，**不单纯是代理造成的**。除了关代理再试，**「粘贴 Cookie 登录」是更稳的那条路**。

### 测试
- 新增 47 例：服务端校验门槛（-101 / 网络 / 风控各自的行为与"不保存"）、清死会话同时清网页 cookie、cookie 串解析容错（乱序 / 换行 / 多余键 / `%2C`）、粘贴 Cookie 的三类失败与成功、看门狗与「清除网页数据后重试」、退出登录的入口显隐与"两处都清干净"。
- 顺带修好两条此前红着的用例（会话失效后的「去登录」引导、收藏夹未登录提示），根因是清 cookie 的 await 让 `-101` 上报多等了一轮。
- 测试总数 2494 → 2541。

### 已知限制
- 「粘贴 Cookie 登录」需要你在电脑浏览器里取一次 cookie（登录 B 站后从开发者工具的 Cookie 列表整段复制），这一步没法由 App 代做。
- WebView 渲染进程崩溃属于外部环境（重型登录页 + 图块内存 / 代理中间人证书），App 侧能做的是"崩了要让你看见、并且有别的路可走"，无法在 App 内根治。
