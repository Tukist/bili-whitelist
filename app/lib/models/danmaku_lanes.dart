...
///   取该侧最外侧的空闲行（顶部优先最上行 / 底部优先最底行，向内侧堆叠）；
///   行全忙 → 丢弃（滚动同理），丢弃计数累加供日志/测试断言。
///
...
class DanmakuLanes {
  ...
  /// 底部弹幕取行：行 0 = 贴显示区底缘的最底一行（与 danmaku_overlay 的
  /// 「行 i y = 底缘 − i×行高 − 文本高」语义一致），优先取最底的空闲行、
  /// 向内侧（上方）堆叠；全忙 → null（丢弃）。
  int? pickBottomLane() {
    for (var i = 0; i < bottomLanes; i++) {
