...
Map<String, dynamic> _videoListBody({
  int code = 0,
  List? vlist,
  int? count,
}) =>
...
void main() {
  ...
  group('searchUpowner', () {
    test('请求参数构造正确（search_type=bili_user + 默认 page=1）'
      ...
      final adapter = _RoutingAdapter({
        ...
        '/x/web-interface/wbi/search/type': () => _userSearchBody(result: [
      ...
      expect(page.upowners[1].face,
          'https://i0.hdslb.com/bfs/face/d.jpg');
  ...
  group('fetchUpownerVideos', () {
    test('请求参数：mid/pn/ps/order + 默认 order=pubdate/ps=20', () async {
      ...
      final req = adapter.requests
          .lastWhere((r) => r.path == '/x/space/wbi/arc/search');
      ...
      // pic // 开头 → 补全 https:
      expect(page.videos.first.cover,
    ...
    test('length 含小时（"1:02:03"）→ 解析为 3723 秒', () async {
      final adapter = _RoutingAdapter({
        ...
        '/x/web-interface/nav': _navBody,
        '/x/space/wbi/arc/search': () => _videoListBody(
              vlist: [_oneVideo(length: '1:02:03')],
              count: 1,
            ),
      });
  ...
  group('fetchUpownerInfo', () {
    test('解析 acc/info：name/face 补全/fans/sign', () async {
      ...
      final req = adapter.requests
          .lastWhere((r) => r.path == '/x/space/wbi/acc/info');
