...
- **我的收藏夹直看（v2.17.7+，浏览方向 MVP）**：首页合集区顶部新增**固定的「收藏夹」卡**（第一层，图标+「我的 B 站收藏」副标，不随合集列表滚动、空名单也显示）→ 点开 = **我的 B 站收藏夹列表**（第二层，`fetchMyFavorites`：封面/名称/N 个视频，下拉刷新）→ 点某收藏夹 = **夹内视频列表**（第三层，`fetchFavoriteVideos` 上拉加载更多）→ **点视频直接播放**（夹内条目无 cid → 播放时 view 接口补齐，走「白名单外可播」模式：**不写 Gist、不自动入库**，进度/历史等常规能力照常）。收藏夹属个人账号数据：**需登录**——首页点卡未登录会先提示并引导登录，总览/视频页遇登录失效（-101）也有「去登录」引导；失效稿件（62002）点播时提示并跳过。**取舍**：夹内非视频条目（type≠2：音频/专栏/合集/剧集等）不展示（B 站合集收藏浏览留后续）；夹内视频仅观看、无加入白名单长按（批量加入走 v2.17.5 收藏夹导入、单条走导入/搜索）；详见 CHANGELOG v2.17.7
