...
- **多 P 选集**：播放页切换分 P
- **竖屏观看布局（v2.17.0+）**：竖屏（非全屏）不再整屏黑底——视频画面**顶部置顶**（按宽高比铺满可用宽，超高视频如 9:16 封顶屏高 60%），下方依次是**视频信息行**（标题 / UP 主名占位 / 时长，UP 入口阶段 C 追加）与**内嵌评论区**（同一屏可滚动）；弹幕 / 字幕 / 手势 / 控制层都绑定在视频画面区内（不覆盖下方内容）；**全屏（横屏）仍为整屏播放布局**（无评论区）。控制层「评论」按钮：**竖屏点击 → 滚动定位到评论区**；**全屏点击 → 打开独立评论页**（原行为，返回后仍全屏续播）
