...
- **播放页 B 站式快捷手势（v2.16.7+）**：**双击**画面播放 / 暂停；**全屏横屏左右滑动** seek（滑动中浮层实时显示进度 `12:34 / 56:78`，松手定位）；**竖屏左半屏上下滑调亮度、右半屏调音量**（原生通道即时生效，应用内亮度仅当前播放页、退出恢复系统亮度；调节带图标 + 百分比浮层）。单击显隐控制层 / 长按 2x / 进度条拖动 / 控制层按钮等既有操作互不干扰（横向与纵向手势按横竖屏分道注册）
