...
- **关注 UP 体系（v2.17.12+）**：App 内统一「**关注 = 加入白名单 UP 主**」语义——**UP 详情页顶部新增「关注 / 已关注」按钮**（未关注点「关注」= 加入白名单；已关注点「已关注」→ 确认后取消关注 = 从白名单移除；播放页 UP 入口、评论里 UP 链接、搜索 UP 结果等**一切进 UP 详情页的途径**都可用），搜索 UP 结果按钮文案统一为「关注 / 已关注」。**UP 主管理页（主页左滑第 2 页）新增「导入我关注的 UP」**：登录 B 站后拉取**自己账号的关注列表**（`x/relation/followings`，需 SESSDATA；分页「加载更多」，最多导入前 200 位并提示总数）→ 勾选 / 全选 → 批量加入白名单——**一次拉 Gist → mid 查重合并 → 一次写回**（非逐条 add），已在白名单的关注项直接标「已关注」不可选，汇总「已添加 X，跳过 Y」；未登录会先提示并引导登录。**取舍**：仅「导入 + App 内关注」一个方向（B 站关注列表 → 白名单）；**不做反向同步**——App 内关注 / 取关只写白名单 Gist、**不回写 B 站关注关系**（`x/relation/modify` 需 csrf 且是官方账号操作，第三方客户端不代写，避免误操作），详见 CHANGELOG v2.17.12
