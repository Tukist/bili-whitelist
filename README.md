...
- **DASH 1080P 播放**：video + audio 双流合并播放；流 URL 过期自动续播。**v2.17.14+ 播放错误自愈**：流 URL 过期（403/404/410/429/5xx）与**瞬时网络错误**（读/建连超时 2001、断连、解析失败）在原生统一归为可自动恢复 → Dart 自动重取流续播（保留位置），续播失败退避 1s→2s→4s 重试有限次，仍失败才提示（手动重试兜底）——网络抖动弹「播放失败」打断观看成为极少情况；数据源超时同步调大（connect 8s→15s、read 8s→20s）减少弱网误判
