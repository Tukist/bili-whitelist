...
- **登录（自动，v2.16.18+）**：WebView 内嵌 B 站官方登录页（短信验证码）——**启动自动处理登录态**：已登录静默恢复，将过期自动用 `refresh_token` **静默续期**（**v2.16.21+ 分档提前续期**：有 refresh_token 距过期 < 15 天即续期——登录一次长期不掉线；无则 7 天），未登录 / 彻底过期自动进入登录页引导（提示条说明"登录一次、之后每次进入自动恢复"）；**v2.16.21+ 关闭登录页 = 明确匿名**——首页与播放页给「未登录仅 720P，去登录解锁 1080P」提示入口，不默认静默降级；登录态下播放自动请求 1080P（qn=80）；**首页不再有常驻登录按钮**，重新登录入口移到管理面板（齿轮 →「B 站账号」）
...
- **首页登录按钮已移除**（旧版右上角常驻"登录（解锁 1080P）"与登录态不同步，已登录也显示、
  每次进 App 都以为要登录）——重新登录 / 换账号入口移到**管理面板**（右上角齿轮 →
...
2. 安装 APK 到手机
3. 打开 App → 齿轮管理面板里填写 GitHub token 与 gist_id（与油猴脚本同一份配置）
4. **登录（v2.16.18 自动）**：未登录首次打开会自动进入 B 站登录页（短信验证码），登录一次后**每次进入自动恢复**；重新登录在齿轮 →「B 站账号」
5. 刷新白名单 → 开始点播
...
| **导入我关注的 UP（v2.17.12+）** | UP 主管理页（主页左滑第 2 页）顶部「导入我关注的 UP」 | 登录 B 站后拉自己账号关注列表（`x/relation/followings`，需 SESSDATA），分页「加载更多」最多前 200 位；勾选/全选 → 批量加入白名单（一次拉 Gist → mid 查重 → 一次写回），已在白名单的关注项标「已关注」自动跳过，汇总「已添加 X，跳过 Y」 |
