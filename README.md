...
- **观看统计（v2.17.9+）**：播放页**按真实观看计时**——仅「正在播放且位置连续前进（0<Δ≤5s）」累计观看秒数（缓冲/暂停/快进快退/拖动 seek/断点跳转不计；**听视频纯音频模式计入**），内存累加约每 10s 批量落盘，按**本地日期**存 shared_preferences（保留近 400 天自动裁剪、损坏容错；数据仅本地不入 Gist）。主页 PageView **新增第 4 页「观看统计」**（主页左滑两页到，顶栏新增观看统计图标直达）：顶部 2×2 总览卡 **今日观看 / 本周观看 / 累计观看 / 最长连续观看天数**（副信息「N 天有观看记录 · 平均每天 X 分钟」）；下方 **GitHub 风格热力图（蓝色主色）**——近 53 周网格（行=周一到周日、列=周，今天在最右列），格子颜色按当天观看分钟分 5 档蓝阶（无观看浅灰 → ≥60 分钟 `#0B5FFF` 深蓝），点/长按格子显示「yyyy-MM-dd · 观看 xx 分钟」，带月份标签 + 少→多图例；无数据显示空态「开始观看后这里会生成你的观看热力」；详见 CHANGELOG v2.17.9
