...
- **评论区（v2.16.15+）**：播放页底部「评论」按钮查看评论区（**只读**，视频 / 番剧通用）——评论列表按热度排序、**上拉翻页**（回传 cursor.next，到底「没有更多了」）、**置顶评论**顶部展示；每条含头像 / 用户名 / 等级角标 / 正文（`<br />` 转行）/ **图片评论**（按原图宽高比显示，动图带角标）/ 点赞 / 时间；**楼中楼**：内嵌至多 3 条预览，点「N 条回复」展开完整楼中楼（分页加载「加载更多回复」）；主评论接口**匿名可得**（无需 WBI），错误分类：评论区已关闭（12002）/ 风控（-412）/ 限流（-352）/ 网络均有友好提示与重试
...
- **代码**：接口 `app/lib/api/bilibili_api.dart`、模型 `app/lib/models/comment.dart`、页面 `app/lib/pages/comment_page.dart`
- **单测**：`app/test/comment_model_test.dart` + `app/test/comment_api_test.dart`；真机集成 `app/integration_test/comment_flow_test.dart`（真实网络：加载 / 楼中楼 / 图片 / 翻页 / 番剧 / 空态）
