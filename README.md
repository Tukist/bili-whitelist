...
- **登录（自动，v2.16.18+）**：WebView 内嵌 B 站官方登录页（短信验证码）——**启动自动处理登录态**：已登录静默恢复（距过期 < 7 天自动续期），未登录自动进入登录页引导（提示条说明"登录一次、之后每次进入自动恢复"）；**首页不再有常驻登录按钮**，重新登录入口移到管理面板（齿轮 →「B 站账号」）
...
- **启动自动登录态处理（v2.16.18）**：进 App 自动检查（首页 `initState`，与按钮无关）——
  `SESSDATA` 有效（距过期 ≥ 7 天）→ **静默恢复**，不弹任何界面；距过期 < 7 天 → 用
  `refresh_token` **静默续期**；无 `SESSDATA`（首次使用 / 登出）/ 彻底过期且续期失败 →
  **自动进入登录页**引导（顶部提示条：登录后自动保存、下次进入自动恢复，无需重复登录）
